...
# Function to create and save Sankey chart
def create_sankey_chart(data, output_path):
    # Define the nodes (unique values in each column)
    source_nodes = data['source'].unique().tolist()
    medium_nodes = data['medium'].unique().tolist()
    campaign_nodes = data['campaign'].unique().tolist()
    device_nodes = data['device'].unique().tolist()
    outcome_nodes = data['outcome'].unique().tolist()
    ...
    source_medium_counts = data.groupby(['source', 'medium']).size().reset_index(name='count')
    ...
    medium_campaign_counts = data.groupby(['medium', 'campaign']).size().reset_index(name='count')
    ...
    campaign_device_counts = data.groupby(['campaign', 'device']).size().reset_index(name='count')
    ...
    device_outcome_counts = data.groupby(['device', 'outcome']).size().reset_index(name='count')
    ...
    try:
        ...
        print(f"Sankey diagram saved as PNG: {output_path}")
        return output_path
    except Exception as e:
        print(f"Could not save Sankey diagram as PNG: {e}")
        return None

# Function to create and save conversion funnel
def create_conversion_funnel(data, output_path):
    # Calculate conversion rates
    total_users = len(data)
    add_to_cart_users = len(data[data['outcome'] == 'Add to Cart'])
    purchase_users = len(data[data['outcome'] == 'Purchase'])
    ...
    # Save as PNG
    try:
        funnel_fig.write_image(output_path)
        print(f"Conversion funnel saved as PNG: {output_path}")
        return output_path
    except Exception as e:
        print(f"Could not save conversion funnel as PNG: {e}")
        return None
...
sankey_path = create_sankey_chart(sankey_data, sankey_image_path)
...
funnel_path = create_conversion_funnel(sankey_data, funnel_image_path)
...
if sankey_path and os.path.exists(sankey_path):
    ...
    pdf.chapter_body('The Sankey diagram could not be generated. Please check the console for errors.')

# Add Conversion Funnel
pdf.chapter_title('Conversion Funnel Visualization')
if funnel_path and os.path.exists(funnel_path):
    ...
    pdf.chapter_body('The conversion funnel could not be generated. Please check the console for errors.')
